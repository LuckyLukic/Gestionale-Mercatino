        ...
        <div class="space-y-12 max-w-7xl mx-auto px-8">
            ...
            <div class="border-b border-gray-900/10 pb-12">
                ...
                <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div class="col-span-2 relative text-left" x-data="{ open: false, selectedOption: '' }">
                        <label for="category" class="block text-sm font-medium leading-6 text-gray-900">Category</label>
                        <div class="mt-2 flex">
                            <input type="text" x-model="selectedOption" wire:model='category'
                        ...
                        <div class="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                            ...
                            <div class="py-1" role="none" x-show="open" @click.away="open=false">
                                <button @click="selectedOption = 'Libro'; open = false" class="text-gray-700 block px-4 py-2 text-sm"
                                    role="menuitem" tabindex="-1" id="menu-item-0">Libro</button>
                                <button @click="selectedOption = 'Gioiello'; open = false" class="text-gray-700 block px-4 py-2 text-sm"
                                    role="menuitem" tabindex="-1" id="menu-item-1">Gioiello</button>
                                <button @click="selectedOption = 'Altro'; open = false" class="text-gray-700 block px-4 py-2 text-sm"
                                    role="menuitem" tabindex="-1" id="menu-item-2">Altro</button>
                            </div>
                        </div>
                    </div>
                    @error('category')
                    ...
                    <div class="col-span-1 col-start-1">
                        ...
                        <div class="mt-2">
                            <input wire:model='quantity' id="quantity" name="quantity" type="quantity"
                                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                            @error('quantity')
                                <em>{{ $message }}</em>
                            @enderror
                    ...
                    <div class="col-span-1 col-start-1">
                        ...
                        <div class="mt-2">
                            <input wire:model='price' type="price" name="price" id="price"
                                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                            @error('price')
                                <em>{{ $message }}</em>
                            @enderror
...
<small>Characters left: <span x-text="255 - $wire.description.length"> </span></small>
                            @error('description')
                                <em>{{ $message }}</em>
                            @enderror
                        </div>
        ...
        <div class="mt-6 max-w-7xl mx-auto px-8 flex items-center justify-end gap-x-6">
            ...
            <button type="submit" wire:confirm='are you sure you want to create this user?'
                ...
                Item</button>
            <button wire:click='clear()' wire:confirm='are you sure you want to delete this user?' type="button"
