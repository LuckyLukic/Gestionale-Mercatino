<div>
    ...
    <div x-data="{ show: @entangle('show') }" x-init="$watch('show', value => { if (value) { setTimeout(() => show = false, 2000); } })" class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 mb-8">
        @if (session()->has('denied'))
            <div x-show="show" x-transition:leave="transition ease-in duration-2000"
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                class="bg-red-900 text-center py-4 lg:px-4">
                <div class="p-2 bg-red-800 items-center text-red-100 leading-none lg:rounded-full flex lg:inline-flex"
                    ...
                    <span class="font-semibold mr-2 text-left flex-auto">{{ session('denied') }}</span>
